# as @e[nbt={Item:{tag:{boom:"timestop"}}}] at @s if entity @s[tag=!boomidset]
...
execute store result score @s boomid run execute if entity @e[tag=timestopeffector]
execute store result score @e[tag=timestopeffector, sort=nearest, limit=1] boomid run execute if entity @e[tag=timestopeffector]
