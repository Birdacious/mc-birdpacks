# as @e[nbt={Item:{tag:{boom:"massive"}}}] at @s if entity @s[tag=!boomidset]
...
execute store result score @s boomid run execute if entity @e[tag=massiveeffector]
execute store result score @e[tag=massiveeffector, sort=nearest, limit=1] boomid run execute if entity @e[tag=massiveeffector]
