# as @e[type=snowball, nbt={Item:{tag:{boom:"buzzing"}}}] at @s if 
...
tag @e[type=minecraft:area_effect_cloud, tag=buzzingeffector, distance=..2, sort=furthest, limit=1] remove detectground

# TELEPORT
# snowball continuously teleports its effector to itself through the air
teleport @e[type=minecraft:area_effect_cloud, tag=buzzingeffector, distance=..2, sort=furthest, limit=1] ~ ~ ~