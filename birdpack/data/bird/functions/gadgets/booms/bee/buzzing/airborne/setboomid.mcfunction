# as @e[type=snowball, nbt={Item:{tag:{boom:"buzzing"}}}] at @s if entity @s[tag=!boomidset]
...
execute store result score @s boomid run execute if entity @e[type=minecraft:area_effect_cloud, tag=buzzingeffector]
execute store result score @e[type=minecraft:area_effect_cloud, tag=buzzingeffector, sort=nearest, limit=1] boomid run execute if entity @e[type=minecraft:area_effect_cloud, tag=buzzingeffector]
