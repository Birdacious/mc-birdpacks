#Calculates the motion of an arrow given the player's current rotation if the player were to release a bow drawn at 8 ticks
...
execute store result entity @e[type=minecraft:area_effect_cloud, tag=laser, distance=..0.00001, limit=1] Motion[0] double -0.00000000208167 run scoreboard players get @s bird.temp
...
scoreboard players operation @s bird.temp *= @s cos.phi
execute store result entity @e[type=minecraft:area_effect_cloud, tag=laser, distance=..0.00001, limit=1] Motion[2] double 0.00000000208167 run scoreboard players get @s bird.temp
#When doing "/execute store," use a scale of *** <arrow speed> / 461168601 *** to multiply by the arrow speed and scale the numbers down in one step!
#   in this case (maximum bow draw, arrow speed = 3) it's 3/461168601 = 0.00000000650521305

#Find the Y motion of a hypothetical arrow and put in AEC's Motion[1]
execute store result entity @e[type=minecraft:area_effect_cloud, tag=laser, distance=..0.00001, limit=1] Motion[1] double -0.000000000447035 run scoreboard players get @s sin.phi
