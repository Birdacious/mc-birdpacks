# as @e[nbt={Item:{tag:{boom:"mega"}}}] at @s if entity @s[tag=!boomidset]
...
execute store result score @s boomid run execute if entity @e[tag=megaeffector]
execute store result score @e[tag=megaeffector, sort=nearest, limit=1] boomid run execute if entity @e[tag=megaeffector]
