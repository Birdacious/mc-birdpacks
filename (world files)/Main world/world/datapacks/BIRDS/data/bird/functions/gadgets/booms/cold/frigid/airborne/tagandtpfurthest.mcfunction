# as @e[nbt={Item:{tag:{boom:"frigid"}}}] at @s if 
...
tag @e[tag=frigideffector, distance=..2, sort=furthest, limit=1] remove detectground

# TELEPORT
# snowball continuously teleports its effector to itself through the air
teleport @e[tag=frigideffector, distance=..2, sort=furthest, limit=1] ~ ~ ~