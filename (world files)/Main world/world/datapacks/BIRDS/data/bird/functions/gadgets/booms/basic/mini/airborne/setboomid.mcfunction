# as @e[nbt={Item:{tag:{boom:"mini"}}}] at @s if entity @s[tag=!boomidset]
...
execute store result score @s boomid run execute if entity @e[tag=minieffector]
execute store result score @e[tag=minieffector, sort=nearest, limit=1] boomid run execute if entity @e[tag=minieffector]
