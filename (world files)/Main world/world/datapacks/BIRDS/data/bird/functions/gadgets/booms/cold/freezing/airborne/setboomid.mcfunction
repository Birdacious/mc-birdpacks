# as @e[nbt={Item:{tag:{boom:"freezing"}}}] at @s if entity @s[tag=!boomidset]
...
execute store result score @s boomid run execute if entity @e[tag=freezingeffector]
execute store result score @e[tag=freezingeffector, sort=nearest, limit=1] boomid run execute if entity @e[tag=freezingeffector]
