# as @e[type=snowball, nbt={Item:{tag:{boom:"bumbling"}}}] at @s if entity @s[tag=!boomidset]
...
execute store result score @s boomid run execute if entity @e[type=minecraft:area_effect_cloud, tag=bumblingeffector]
execute store result score @e[type=minecraft:area_effect_cloud, tag=bumblingeffector, sort=nearest, limit=1] boomid run execute if entity @e[type=minecraft:area_effect_cloud, tag=bumblingeffector]
