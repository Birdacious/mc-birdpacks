# as @e[nbt={Item:{tag:{boom:"medium"}}}] at @s if 
...
tag @e[tag=mediumeffector, distance=..2, sort=furthest, limit=1] remove detectground

# TELEPORT
# snowball continuously teleports its effector to itself through the air
teleport @e[tag=mediumeffector, distance=..2, sort=furthest, limit=1] ~ ~ ~